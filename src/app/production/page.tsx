...
export default function ProductionFlowPage() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    const totalQuantity = values.rawMaterials.reduce((sum, material) => sum + material.quantity, 0)
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      filteredProductions.map((p) => ({
        ...
        "Batch No": p.batchNo,
        "Raw Materials": p.rawMaterials.map((m) => `${m.name} (${m.quantity} kg/L)`).join(", "),
            ...
            <TableBody>
              {filteredProductions.map((production) => (
                <TableRow key={production.id}>
                  <TableCell>{format(production.dateOfManufacturing, "PPP")}</TableCell>
                  <TableCell>{production.batchNo}</TableCell>
                  <TableCell>
                    {production.rawMaterials.map((material, index) => (
                      <div key={index}>
                        {material.name}: {material.quantity} kg/L @ ₹{material.pricePerUnit}/unit
                      </div>
                    ))}
                  </TableCell>
                  <TableCell>{production.totalQuantity.toFixed(2)} Kgs/Ltrs</TableCell>
                  <TableCell>₹{production.totalRawMaterialCost.toFixed(2)}</TableCell>
                  <TableCell>₹{production.pricePerUnit.toFixed(2)}/kg or L</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
